...
from tensorflow.keras.applications import (VGG16, ResNet50, MobileNet,
                                            ...
                                            EfficientNetB6, EfficientNetB7, AlexNet, SqueezeNet)
...
from tensorflow.keras.applications.alexnet import preprocess_input as alexnet_preprocess_input
...
def extract_and_save_feature_extractor(image_folder_path, model_name, save_path):
    ...
    model_dict = {
        ...
        'inceptionv4': (InceptionV4, inception_v3_preprocess_input),  # Note: same preprocessing as InceptionV3
        ...
        'alexnet': (AlexNet, alexnet_preprocess_input),
        'squeezenet': (SqueezeNet, squeezenet_preprocess_input),
    }

    if model_name.lower() not in model_dict:
        raise ValueError(f"Unsupported model: {model_name}. Supported models are: {', '.join(model_dict.keys())}")

    # Load the pre-trained model (excluding the classification layers)
    base_model = model_dict[model_name.lower()][0](weights='imagenet', include_top=False)
    ...
    for file_name in image_file_names:
        ...
        features = feature_extractor.predict(np.expand_dims(preprocessed_img, axis=0))  # Extract features
...
def test_feature_extraction(image_folder_path, save_dir):
    model_names = ['vgg16', 'resnet50', 'mobilenet', 'inceptionv3', 'inceptionv4',
    ...
    for model_name in model_names:
        save_path = os.path.join(save_dir, f"{model_name}_feature_extractor.h5")
        extract_and_save_feature_extractor(image_folder_path, model_name, save_path)

        # Verify if the model is saved correctly
        assert os.path.exists(save_path), f"Failed to save the {model_name} feature extractor model."

        # Load the saved model to check if it is a valid Keras model
        try:
            loaded_model = load_model(save_path)
            assert isinstance(loaded_model, tf.keras.Model), f"The {model_name} saved model is not a valid Keras model."
        except Exception as e:
            assert False, f"Error while loading the {model_name} saved model: {e}"

        print(f"{model_name} feature extractor model saved successfully and passed validation.")
